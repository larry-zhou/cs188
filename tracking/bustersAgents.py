...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for d in livingGhostPositionDistributions:
            # DicreteDistributions have argmax function
            mostlikely.append(d.argMax())
            
        closestghost = min(mostlikely)
        
        actions = []
        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successorPosition, closestghost)
            actions.append((distance, action))
            
        return min(actions, key = lambda t: t[0])[1]
