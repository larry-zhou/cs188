...
def constructBayesNet(gameState):
    ...
    "*** YOUR CODE HERE ***"
    # append edges in tuple form (from, to)
    ...
    for housePos in gameState.getPossibleHouses():
            for obsPos in gameState.getHouseWalls(housePos):
                ...
                for assignment in obsFactor.getAllPossibleAssignmentDicts():
                    ...
                    foodBottomLeft = assignment[FOOD_HOUSE_VAR] == BOTTOM_LEFT_VAL
                    foodTopLeft = assignment[FOOD_HOUSE_VAR] == TOP_LEFT_VAL
                    foodBottomRight = assignment[FOOD_HOUSE_VAR] == BOTTOM_RIGHT_VAL
                    foodTopRight = assignment[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL
                    ghostBottomLeft = assignment[GHOST_HOUSE_VAR] == BOTTOM_LEFT_VAL
                    ghostTopLeft = assignment[GHOST_HOUSE_VAR] == TOP_LEFT_VAL
                    ghostBottomRight = assignment[GHOST_HOUSE_VAR] == BOTTOM_RIGHT_VAL
                    ghostTopRight = assignment[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL
                    #check for food house first because it takes precedent over ghost house
                    if housePos == bottomLeftPos and foodBottomLeft or \
                        housePos == topLeftPos and foodTopLeft or \
                        housePos == bottomRightPos and foodBottomRight or \
                        housePos == topRightPos and foodTopRight:
                        if red:
                            prob = PROB_FOOD_RED
                        elif blue:
                            prob = 1 - PROB_FOOD_RED
                    #check if its a ghost house next
                    elif housePos == bottomLeftPos and ghostBottomLeft or \
                        housePos == topLeftPos and ghostTopLeft or \
                        housePos == bottomRightPos and ghostBottomRight or \
                        housePos == topRightPos and ghostTopRight:
                        if red:
                            prob = PROB_GHOST_RED
                        elif blue:
                            prob = 1 - PROB_GHOST_RED
                    # if obs is neither ghosthouse or foodhouse, then the probability is 1 when the wallcolor is None. 0
                    else:
                        if red:
                            prob = 0
                        elif blue:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    """
    "*** YOUR CODE HERE ***"
    something = inference.inferenceByVariableElimination(bayesNet, FOOD_HOUSE_VAR, evidence, eliminationOrder)
    mostLikely = None
    highestProb = 0
    for assignment in something.getAllPossibleAssignmentDicts():
        if something.getProbability(assignment) > highestProb:
            mostLikely = assignment
            highestProb = something.getProbability(assignment)
    print mostLikely
    return mostLikely
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        First look at computeEnterValues. This function computes the expected value of entering the left and right houses. 
        Again, you can use the inference code you already wrote to do all the heavy lifting here.
        First compute p(foodHouse = topLeft and ghostHouse = topRight | evidence) and p(foodHouse = topRight and ghostHouse = topLeft | evidence). 
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        leftPROB = houseMarginals.getProbability(left)
        rightPROB = houseMarginals.getProbability(right)
        leftExpectedValue = leftExpectedValue + (leftPROB * WON_GAME_REWARD + rightPROB * GHOST_COLLISION_REWARD)
        rightExpectedValue = rightExpectedValue + (rightPROB * WON_GAME_REWARD + leftPROB * GHOST_COLLISION_REWARD)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        explorations = self.getExplorationProbsAndOutcomes(evidence)
        for e in explorations:
            prob, explorationEvidence = e
