...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        "*** YOUR CODE HERE ***"
        # look at inferenceByEnumeration
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for elem in eliminationOrder:
            # Interleave join
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, elem)
            jfu = joinedFactor.unconditionedVariables()
            if not (len(jfu) == 1):
                # Eliminate and update
                joinedFactor = eliminate(joinedFactor, elem)
                currentFactorsList.append(joinedFactor)
        # Normalize to make sure sum of probabilities is one        
